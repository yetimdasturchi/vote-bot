...
class Xlsxgen {
    ...
    public function __construct() {
        ...
        $this->XF = [
            ...
            [self::N_NORMAL, self::A_DEFAULT, self::F_NORMAL, self::FL_GRAY_125, 0, 0, '', 0],
        ];
...
<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>'
    ...
    protected function _write($fh) {
        $dirSignatureE = "\x50\x4b\x05\x06";
        $zipComments = 'Generated by yetimdasturchi';
        if (!$fh) {
            return false;
        }
        $cdrec = '';
    ...
    protected function _writeEntry($fh, &$cdrec, $cfilename, $data) {
        ...
        fwrite($fh, pack('s', $e['vneeded']));
        fwrite($fh, pack('s', $e['bitflag']));
        fwrite($fh, pack('s', $e['cmethod']));
        fwrite($fh, pack('s', $e['modtime']));
        fwrite($fh, pack('s', $e['moddate']));
        fwrite($fh, pack('V', $e['crc_32']));
        fwrite($fh, pack('I', $e['comsize']));
        fwrite($fh, pack('I', $e['uncsize']));
        fwrite($fh, pack('s', mb_strlen($cfilename, '8bit')));
        fwrite($fh, pack('s', 0));
        fwrite($fh, $cfilename);

        fwrite($fh, $zdata);

        $e['external_attributes'] = (substr($cfilename, -1) === '/' && !$zdata) ? 16 : 32;
        $e['comments'] = '';
        ...
        $cdrec .= pack('v', $e['modtime']);
        $cdrec .= pack('v', $e['moddate']);
                    ...
                    if (is_string($v)) {
                        if ($v[0] === "\0") {
                            ...
                            if ($N) {
                                ...
                                $cv = round($m[1] / 100, 2);
                                $N = self::N_PERCENT_INT;
                            } elseif (preg_match('/^([-+]?\d+\.\d+)%$/', $v, $m)) {
                                $cv = round($m[1] / 100, 4);
                                $N = self::N_PRECENT_DEC;
                                ...
                                $N = ((int)$m[1] === 0) ? self::N_TIME : self::N_DATETIME;
                            } elseif (preg_match('/^(\d\d)\/(\d\d)\/(\d\d\d\d) (\d\d):(\d\d):(\d\d)$/', $v, $m)) {
